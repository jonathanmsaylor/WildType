...
Press **Tab**, then choose **Chronicle** at the bottom of the existing journal's right panel. Three roomier records per page show the controlled creature, its actual ancestors, descendants and **descendants of creatures you previously controlled**. That retains earlier children and their branches after control transfer. Each record keeps the **name + stable ID + generation**, relationship to the named current creature, living adult/juvenile or recorded deceased status, parent IDs, **children born**, and current age or recorded lifespan. Matching first names or full names remain distinct by ID. **Next page** cycles through the bounded archive view; **Living descendants** returns to the four-card selector, now explicitly headed **Living descendants of #ID · Name**.

Only living **descendants of the current creature** have selectable control cards. Other living family members in Chronicle have **Locate** and say **locate only**. Siblings (including half-siblings) are labeled **Sibling**, not descendant; more distant collateral branches say **Other family branch** with exact parent IDs. Yourself, deceased and unavailable entries say **record only**. Locate retains its existing pulse, does not transfer control or walk, and remains disabled on the death menu. Locating a sibling or grandchild never grants direct-child care. Taking control resets the view to the new creature's living descendants while preserving earlier family branches in Chronicle. The view follows actual ancestry and controlled-history IDs, not merely a common founder label. A child's other parent remains identified by ID even when outside the view.
