...
A Unity implementation, independent of the earlier Godot project. **Current milestone: gameplay clarity and readable stats**, on `milestone-gameplay-clarity`, based on pushed painterly prototype `168d0f0`. `main` is deliberately unchanged. Explore, eat, reproduce, recognize and locate your family, read their recorded lives, optionally support your children, and continue through living descendants in a bounded, roughly 250 m-wide Creature-stage ecosystem. Start with twelve autonomous herbivores; total living population is capped at 24.
...
3. After a birth, name your child or keep its default. **Tab → Living descendants** follows the creature you currently control. **Chronicle** also retains earlier controlled branches.
4. **Highlight · Locate** reveals a living relative while you remain in your creature. **Take control** is a separate, explicitly labeled button available only for your living descendants. A sibling can be highlighted, not controlled or cared for.
5. Optional **R share** helps a nearby juvenile direct child. The prompt quotes your real energy cost and the child's real gain. It neither heals nor guarantees survival.
...
5. Press **Tab** (gamepad north button) to open the paused family journal. Names, IDs, generations, relationships, parent IDs and life state appear on three readable cards per page. **Details** exposes age, resources, region/distance, inherited adult values, exact genes and birth-time parent comparisons. **Take control** switches to a living descendant; clicking the card text does not. The former player becomes AI if alive. **Highlight · Locate** briefly reveals a relative without transferring control. No healing, aging reset or resource refill occurs.
6. Death pauses the world and offers the same descendant choices. With no descendants, restart or reseed explicitly; the game never creates a hidden replacement. Taking a branch means future **control choices** are descendants of that newly controlled creature, not its siblings or ancestors. Earlier controlled creatures' children remain discoverable in **Chronicle**, including siblings and their branches; living family records have Locate without granting control or care.
...
Both family views use **three cards per page** with 23-point text and separate Highlight, Take control and Details buttons. Previous/Next page preserves access to every retained entry. Buttons are labeled rather than distinguished by color alone.

### Lineage chronicle

Press **Tab**, then choose **Chronicle** in the left menu. Three records per page show the controlled creature, its actual ancestors, descendants and **descendants of creatures you previously controlled**. That retains earlier children and their branches after control transfer. Each record keeps the **name + stable ID + generation**, relationship to the named current creature, living adult/juvenile or recorded deceased status, parent IDs, **children born**, and current age or recorded lifespan. Matching names remain distinct by ID. **Previous/Next page** browses the bounded archive; **Living descendants** returns to the current creature's descendant list.

Only living **descendants of the current creature** have an enabled **Take control** button. Other living family members in Chronicle have **Highlight · Locate** and say **highlight only**. Siblings (including half-siblings) are labeled **Sibling**, not descendant; more distant collateral branches say **Other family branch** with exact parent IDs. Yourself, deceased and unavailable entries say **record only**. Locate retains its existing pulse, does not transfer control or walk, and is unavailable on the death menu. Locating a sibling or grandchild never grants direct-child care. Taking control resets the view to the new creature's living descendants while preserving earlier family branches in Chronicle. The view follows actual ancestry and controlled-history IDs, not merely a common founder label. A child's other parent remains identified by ID even when outside the view.
